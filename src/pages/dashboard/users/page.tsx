...
const UsersPage = () => {
  ...
  const { data: userData, isLoading } = useQuery({ queryKey: ["users"], queryFn: () => axiosInstance.get(endpoints.user.list) });
  ...
  return (
    <Styled>
      ...
      <SimpleBar style={{ maxWidth: "100%" }}>
        <Table loading={isLoading} columns={columns} dataSource={filteredUsers} pagination={false} />
      </SimpleBar>
    </Styled>
  );
};
